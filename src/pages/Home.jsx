...
function Home() {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/v1/getCompany');
  ...
  const handleCompanyCreate = async (newCompany) => {
    try {
      const response = await fetch('http://localhost:5000/api/v1/createCompany', {
        method: 'POST',
  ...
  const handleCompanyUpdate = async (updatedCompany) => {
    try {
      const response = await fetch(`http://localhost:5000/api/v1/updateCompany/${updatedCompany._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCompanyDelete = async (companyId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/v1/deleteCompany/${companyId}`, {
        method: 'DELETE',
      });
